...
PYBIND11_MODULE(abspy, m)
{
	...
	m.doc() = "C++ binding for ABS";

	py::class_<CurveND<2>>(m, "Curve2d")
		.def_readwrite("transform", &CurveND<2>::transform)
		.def_readwrite("interval", &CurveND<2>::interval)
		.def_readwrite("type", &CurveND<2>::type)
		.def("sample", [](const CurveND<2> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 2);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate(params(i));
        }
        return samples; }, "Sample points on the curve given parameter values", py::arg("params"))
		.def("first_derivative", [](const CurveND<2> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 2);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate_derivative(params(i));
        }
        return samples; }, "Sample points on the derivative curve given parameter values", py::arg("params"))
		.def("second_derivative", [](const CurveND<2> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 2);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate_2nd_derivative(params(i));
        }
        return samples; }, "Sample points on the second derivative curve given parameter values", py::arg("params"));

	py::class_<CurveND<3>>(m, "Curve3d")
		.def_readwrite("transform", &CurveND<3>::transform)
		.def_readwrite("interval", &CurveND<3>::interval)
		.def_readwrite("type", &CurveND<3>::type)
		.def("sample", [](const CurveND<3> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 3);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate(params(i));
        }
        return samples; }, "Sample points on the curve given parameter values", py::arg("params"))
		.def("first_derivative", [](const CurveND<3> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 3);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate_derivative(params(i));
        }
        return samples; }, "Sample points on the derivative curve given parameter values", py::arg("params"))
		.def("second_derivative", [](const CurveND<3> &self, const Eigen::VectorXd &params) {
        Eigen::MatrixXd samples(params.size(), 3);
        for (int i = 0; i < params.size(); ++i) {
            samples.row(i) = self.curve->evaluate_2nd_derivative(params(i));
        }
	...
	py::class_<Patch>(m, "Surface")
		.def_readwrite("transform", &Patch::transform)
		.def_readwrite("domain", &Patch::domain)
		.def_readwrite("type", &Patch::type)
		.def("sample", [](const Patch &self, const Eigen::MatrixXd &params) {
        Eigen::MatrixXd samples(params.rows(), 3);
        for (int i = 0; i < params.rows(); ++i) {
            samples.row(i) = self.patch->evaluate(params(i, 0), params(i, 1));
        }
        return samples; }, "Sample points on the surface given parameter values", py::arg("params"))
		.def("first_derivative", [](const Patch &self, const Eigen::MatrixXd &params) {
        Eigen::MatrixXd du(params.rows(), 3);
        Eigen::MatrixXd dv(params.rows(), 3);
        for (int i = 0; i < params.rows(); ++i) {
            du.row(i) = self.patch->evaluate_derivative_u(params(i, 0), params(i, 1));
            dv.row(i) = self.patch->evaluate_derivative_v(params(i, 0), params(i, 1));
        }
        return std::make_tuple(du, dv); }, "Sample points on the derivative surface given parameter values", py::arg("params"))
		.def("second_derivative", [](const Patch &self, const Eigen::MatrixXd &params) {
        Eigen::MatrixXd duu(params.rows(), 3);
        Eigen::MatrixXd dvv(params.rows(), 3);
        Eigen::MatrixXd duv(params.rows(), 3);
        for (int i = 0; i < params.rows(); ++i) {
            duu.row(i) = self.patch->evaluate_2nd_derivative_uu(params(i, 0), params(i, 1));
            dvv.row(i) = self.patch->evaluate_2nd_derivative_vv(params(i, 0), params(i, 1));
            duv.row(i) = self.patch->evaluate_2nd_derivative_uv(params(i, 0), params(i, 1));
        }
        return std::make_tuple(duu, dvv, duv); }, "Sample points on the second derivative surface given parameter values", py::arg("params"));

	py::class_<Part>(m, "Part")
		.def_readwrite("vertices", &Part::vertices)
		.def_readwrite("meshes", &Part::meshes)
		.def_readwrite("bbox", &Part::bbox)
		.def_readwrite("curves2d", &Part::curves2d)
		.def_readwrite("curves3d", &Part::curves3d)
		.def_readwrite("surfaces", &Part::surfaces);
	...
	m.def(
		...
		[](const std::string &path) {
			...
			read_parts(path, parts);
			return parts;
		},
		"Read CAD geometry from HDF5 file",
		py::arg("path"));
}