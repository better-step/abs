...
class SurfaceSampler(Sampler):
    ...
    def sample(self, surface):
        ...
        if self.method == 'uniform' or self.method == 'random':
            return self._sampling_based_on_surface(surface)
        else:
            raise ValueError(f"Invalid sampling method: {self.method}")

    def _sampling_based_on_surface(self, surface):
        """
    ...
    def _uniform_or_random_sampling(self, trim_domain_u, trim_domain_v, num_points_u, num_points_v, method='uniform'):

        if method == 'uniform': # Generate uniformly spaced values in u and v dimensions
        ...
        elif method == 'random':  # Generate randomly spaced values in u and v dimensions
            u_values = np.random.uniform(low=trim_domain_u[0], high=trim_domain_u[1], size=num_points_u)
            v_values = np.random.uniform(low=trim_domain_v[0], high=trim_domain_v[1], size=num_points_v)
        else:
    ...
    def sample_cylinder(self, patch):
        ...
        return self._uniform_or_random_sampling(trim_domain_u, trim_domain_v, num_points_u, num_points_v, self.method)
    ...
    def _other_sampling(self, surface):
        ...
        num_points_u = max(int(abs(u_range[1] - u_range[0]) / self.spacing), 1)
        num_points_v = max(int(abs(v_range[1] - v_range[0]) / self.spacing), 1)
        ...
        #
        # return uv_values
        return self._uniform_or_random_sampling(u_range, v_range, num_points_u, num_points_v, self.method)
